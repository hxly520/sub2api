...
func TestCodexModelsAPIKeyManifestTriesAnotherAccountAfterPermanentStatus(t *testing.T) {
	statuses := []int{
...
func TestCodexModelsAPIKeyManifestTriesAnotherAccountAfterConfigurationError(t *testing.T) {
	handler, upstream, groupID := newCodexModelsFailoverTestHandler(http.StatusServiceUnavailable)
...
func TestCodexModelsAPIKeyManifestFallsBackToBuiltinCatalogWhenAccountsAreExhausted(t *testing.T) {
	handler, upstream, groupID := newCodexModelsFailoverTestHandler(http.StatusServiceUnavailable)
	...
	if body := recorder.Body.String(); body != `{"models":[]}` {
		t.Fatalf("body: got %q, want built-in catalog fallback", body)
	}
}
