...
// KeyingPayNotify handles KeyingPay V2 signed GET/POST notifications.
// GET/POST /api/v1/payment/webhook/keyingpay
func (h *PaymentWebhookHandler) KeyingPayNotify(c *gin.Context) {
	h.handleNotify(c, payment.TypeKeyingPay)
}
...
func extractOutTradeNo(rawBody, providerKey string) string {
	switch providerKey {
	case payment.TypeEasyPay, payment.TypeKeyingPay, payment.TypeAlipay:
