...
const (
	...
	EndpointContentTasks      = "/contents/generations/tasks"
...
func NormalizeInboundEndpoint(path string) string {
	...
	case strings.Contains(path, EndpointVideoGenerations) || strings.Contains(path, "/video/generations"):
		return EndpointVideoGenerations
	case strings.Contains(path, EndpointVideos) || isBareOrSubpathOf(strings.TrimRight(strings.TrimSpace(path), "/"), "/videos"):
...
func DeriveUpstreamEndpoint(inbound, rawRequestPath, platform string) string {
	...
	case service.PlatformOpenAI, service.PlatformGrok:
		if inbound == EndpointEmbeddings || inbound == EndpointImagesGenerations || inbound == EndpointImagesEdits || inbound == EndpointVideosGenerations || inbound == EndpointVideoGenerations || inbound == EndpointVideos || inbound == EndpointContentTasks {
			return inbound
		}
