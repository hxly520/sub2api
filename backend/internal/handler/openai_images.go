...
func (h *OpenAIGatewayHandler) Images(c *gin.Context) {
	...
	requestCtx = service.WithOpenAIAccountScheduleProfile(requestCtx, service.NewOpenAIAccountScheduleProfile(requestModel, GetInboundEndpoint(c), ""))
		...
		if err != nil {
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &imageUpstreamErr) {
					...
					h.reportOpenAIAccountScheduleResult(c, account, requestModel, !retryableServerError, nil)
