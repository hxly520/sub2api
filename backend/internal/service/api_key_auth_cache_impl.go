...
// v21 adds group long-context/model pricing fields. Bumping the version forces
// stale v20 snapshots (which silently dropped those fields) back to the DB.
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	snapshot := &APIKeyAuthSnapshot{
		...
		LinkState:          apiKey.LinkState,
		LinkRateMultiplier: apiKey.LinkRateMultiplier,
		LinkOriginalDebit:  apiKey.LinkOriginalDebit,
		LinkTotalFunded:    apiKey.LinkTotalFunded,
		LinkTotalRefunded:  apiKey.LinkTotalRefunded,
		LinkReservedAmount: apiKey.LinkReservedAmount,
		LinkConcurrency:    apiKey.LinkConcurrency,
		LinkRPMLimit:       apiKey.LinkRPMLimit,
		LinkActivatedAt:    apiKey.LinkActivatedAt,
		LinkRevokedAt:      apiKey.LinkRevokedAt,
		LinkFrozenReason:   apiKey.LinkFrozenReason,
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		LinkRateMultiplier: snapshot.LinkRateMultiplier,
		LinkOriginalDebit:  snapshot.LinkOriginalDebit,
		LinkTotalFunded:    snapshot.LinkTotalFunded,
		LinkTotalRefunded:  snapshot.LinkTotalRefunded,
		LinkReservedAmount: snapshot.LinkReservedAmount,
		LinkConcurrency:    snapshot.LinkConcurrency,
		LinkRPMLimit:       snapshot.LinkRPMLimit,
		LinkActivatedAt:    snapshot.LinkActivatedAt,
		LinkRevokedAt:      snapshot.LinkRevokedAt,
		LinkFrozenReason:   snapshot.LinkFrozenReason,
