...
func TestNormalizeVideoModelPricesKeepsPrivate4KAndDropsUnknownResolutions(t *testing.T) {
	t.Parallel()
	// The private video API supports 4K. Truly unknown tiers must still be
	// dropped instead of collapsing into 480p.
	...
	// A private 4K-only model remains valid.
	require.Equal(t, map[string]map[string]float64{
		VideoPriceFamilyGrokImagineVideo: {VideoBillingResolution4K: 0.50},
	}, NormalizeVideoModelPrices(map[string]map[string]float64{
