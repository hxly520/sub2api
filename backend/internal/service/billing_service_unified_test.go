...
func TestCalculateCostUnified_GPT56AccountLongContextWithoutChannelPricing(t *testing.T) {
	...
	require.InDelta(t, float64(tokens.InputTokens)*5e-6*2, cost.InputCost, 1e-12)
	require.InDelta(t, float64(tokens.CacheCreationTokens)*6.25e-6*2, cost.CacheCreationCost, 1e-12)
	require.InDelta(t, float64(tokens.CacheReadTokens)*0.5e-6*2, cost.CacheReadCost, 1e-12)
	require.InDelta(t, float64(tokens.OutputTokens)*30e-6*1.5, cost.OutputCost, 1e-12)
	require.InDelta(t, cost.TotalCost*0.12, cost.ActualCost, 1e-12)
}
