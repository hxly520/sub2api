...
func (i *PluginPackageInstaller) Install(ctx context.Context, reader io.Reader, installedBy *int64) (*PluginInstallation, error) {
	...
	// Close the archive before renaming the uploaded file. Windows keeps the
	// underlying descriptor locked until the zip reader is closed.
	if err := archive.Close(); err != nil {
		return nil, fmt.Errorf("close plugin archive before commit: %w", err)
	}
	archiveClosed = true
