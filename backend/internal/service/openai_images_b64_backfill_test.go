...
func TestOpenAIGatewayServiceForwardImages_APIKeyBackfillsB64JSONFromURL(t *testing.T) {
	...
	svc := &OpenAIGatewayService{cfg: b64BackfillEdgeConfig()}
		...
		responses: []*http.Response{
			{
				...
				Body: io.NopCloser(strings.NewReader(
					`{"created":1710000000,"data":[{"url":"https://1.1.1.1/cat.png","revised_prompt":"a cat"}],"usage":{"input_tokens":10,"output_tokens":20}}`,
				)),
			},
	...
	require.Equal(t, "https://1.1.1.1/cat.png", upstream.requests[1].URL.String())
	...
	require.True(t, strings.HasPrefix(gjson.Get(rec.Body.String(), "data.0.url").String(), "https://video.52token.example/v1/image-content/"))
...
func TestOpenAIGatewayServiceForwardImages_APIKeyLeavesURLOnlyResponseWhenDisabled(t *testing.T) {
	...
	upstreamBody := `{"created":1710000000,"data":[{"url":"https://1.1.1.1/cat.png"}]}`
	upstream := &httpUpstreamRecorder{
		responses: []*http.Response{
			{
				StatusCode: http.StatusOK,
...
func b64BackfillEdgeConfig() *config.Config {
	return &config.Config{Gateway: config.GatewayConfig{VideoProxy: config.VideoProxyConfig{
		Mode: config.VideoProxyModeEdge, EdgeBaseURL: "https://video.52token.example", EncryptionKey: strings.Repeat("33", 32), TokenTTLSeconds: 3600,
	}}}
}
