...
func (e *OpenAIImagesUpstreamError) clientMessage() string {
	...
	if trimmed := strings.TrimSpace(e.Message); trimmed != "" {
		if sanitized := sanitizeClientUpstreamErrorMessage(trimmed); sanitized != "" {
			return sanitized
		}
	}
...
// Plain text can also be a capability fallback, so classify it before
...
func (s *OpenAIGatewayService) handleOpenAIImagesOAuthResponseError(
	...
	if upstreamErr.Code == "image_generation_unavailable" {
		...
		failoverErr.MediaOutcomeKnownFailed = upstreamErr.MediaOutcomeKnownFailed
		return failoverErr
	}
	...
	failoverErr.MediaOutcomeKnownFailed = upstreamErr.MediaOutcomeKnownFailed ||
		IsDefinitiveMediaGenerationFailure(upstreamErr.StatusCode, responseBody)
	return failoverErr
}
