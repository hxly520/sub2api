...
func (s *OpenAIGatewayService) proxyOpenAIWSHTTPBridgeTurn(
	...
	pendingSSEEventType := ""
	flushPendingClientMessages := func() error {
		...
		for _, message := range messages {
			if err := writeClientMessage(message); err != nil {
				...
				return fmt.Errorf("write staged websocket event: %w", err)
			}
	...
	for scanner.Scan() {
		...
		if eventType == "error" || eventType == "response.failed" {
			...
			// API-key bridge clients can disappear after receiving response.created
			// while the detached upstream continues. Probe staged metadata before a
			// terminal stream error so a closed client cannot trigger account replay.
			if account.Type == AccountTypeAPIKey && len(pendingClientMessages) > 0 {
				if flushErr := flushPendingClientMessages(); flushErr != nil {
					return nil, wrapOpenAIWSIngressTurnError("write_client", flushErr, wroteDownstream)
				}
			}
