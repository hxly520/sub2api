...
type cancelAwareOpenAIImagesUpstream struct {
	HTTPUpstream
	started chan struct{}
	release chan struct{}
}

func (u *cancelAwareOpenAIImagesUpstream) Do(req *http.Request, _ string, _ int64, _ int) (*http.Response, error) {
	close(u.started)
	<-u.release
...
func TestOpenAIGatewayServiceForwardImages_APIKeyNonStreamDrainsAfterClientCancel(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"gpt-image-2","prompt":"draw a cat","response_format":"b64_json"}`)
	...
	upstream := &cancelAwareOpenAIImagesUpstream{
	...
	require.NoError(t, err)
	account := &Account{
		ID: 6, Name: "openai-apikey", Platform: PlatformOpenAI, Type: AccountTypeAPIKey,
	...
	go func() {
		result, forwardErr := svc.ForwardImages(requestCtx, c, account, body, parsed, "")
	...
	require.Equal(t, 1, forwarded.result.ImageCount)
}
...
func TestOpenAIGatewayServiceForwardImages_APIKeyClassifiesMediaFailureOutcome(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "explicit insufficient balance", statusCode: http.StatusForbidden, body: `{"message":"Insufficient account balance"}`, known: true},
		{name: "provider queue cancelled", statusCode: http.StatusBadGateway, body: `{"message":"管理员取消：生图池排队任务已清空"}`, known: true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			account := &Account{
				ID: 7, Name: "openai-apikey-error", Platform: PlatformOpenAI, Type: AccountTypeAPIKey,
				Credentials: map[string]any{"api_key": "test-api-key", "base_url": "https://image-upstream.example/v1"},
			}
			...
			var failoverErr *UpstreamFailoverError
			require.ErrorAs(t, err, &failoverErr)
			require.Equal(t, tt.known, failoverErr.MediaOutcomeKnownFailed)
		})
	}
}
