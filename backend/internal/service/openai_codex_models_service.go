...
// served. APIKeyOnly is set only after every eligible API-key manifest account
// has been exhausted, allowing Codex to fall back to its built-in catalog.
...
func (s *OpenAIGatewayService) SelectAccountForCodexModels(ctx context.Context, groupID *int64) (*CodexModelsAccountSelection, error) {
	return s.SelectAccountForCodexModelsWithExclusions(ctx, groupID, nil)
}

// SelectAccountForCodexModelsWithExclusions selects a manifest account while
// preserving OAuth/setup-token preference and excluding failed attempts.
func (s *OpenAIGatewayService) SelectAccountForCodexModelsWithExclusions(ctx context.Context, groupID *int64, excludedIDs map[int64]struct{}) (*CodexModelsAccountSelection, error) {
	...
	return s.selectAccountForCodexModelsWithExclusions(ctx, accounts, groupID, excludedIDs)
}
...
func (s *OpenAIGatewayService) selectAccountForCodexModels(ctx context.Context, accounts []Account) (*CodexModelsAccountSelection, error) {
	return s.selectAccountForCodexModelsWithExclusions(ctx, accounts, nil, nil)
}

func (s *OpenAIGatewayService) selectAccountForCodexModelsWithExclusions(ctx context.Context, accounts []Account, groupID *int64, excludedIDs map[int64]struct{}) (*CodexModelsAccountSelection, error) {
	var firstOAuth *Account
	var firstBrokenOAuth *Account
	var firstAPIKey *Account
	hasAPIKey := false
	...
	for i := range accounts {
		...
		candidate = s.recheckSelectedOpenAIAccountFromDB(ctx, candidate, groupID, PlatformOpenAI, "", false, "")
		if candidate == nil {
			continue
		}
		...
		if candidate.Type == AccountTypeAPIKey {
			hasAPIKey = true
			if _, excluded := excludedIDs[candidate.ID]; !excluded && firstAPIKey == nil {
				firstAPIKey = candidate
			}
			continue
		}
	...
	if firstOAuth != nil {
		return &CodexModelsAccountSelection{Account: firstOAuth}, nil
	}
	...
	if firstAPIKey != nil {
		return &CodexModelsAccountSelection{Account: firstAPIKey}, nil
	}
	if hasAPIKey {
