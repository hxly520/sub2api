...
func TestOpenAIGatewayService_Forward_HTTPPoolModeDoesNotReplayInvalidEncryptedContent(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstream := &httpUpstreamSequenceRecorder{
		responses: []*http.Response{
			...
			},
			{
				StatusCode: http.StatusOK,
				Header:     http.Header{"Content-Type": []string{"application/json"}},
				Body:       io.NopCloser(strings.NewReader(`{"id":"must_not_be_used","usage":{"input_tokens":1,"output_tokens":1}}`)),
			},
		},
	}
