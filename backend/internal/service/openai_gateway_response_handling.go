...
func sanitizeOpenAIResponseFailedEventForClient(payload []byte, eventType string, clientOutputStarted bool) ([]byte, bool) {
	...
	updated := payload
	// Keep the official response.failed shape while sanitizing only the
	// client-facing message.  Upstream URLs, credentials, and stack details
	// must never be echoed to API clients; operators still retain the original
	// diagnostic in the internal/ops log path.
	for _, path := range []string{"response.error.message", "error.message", "message"} {
