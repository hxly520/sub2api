...
func newCodexModelsSelectionAPIKeyTestAccount(id int64) Account {
	return Account{
		ID:          id,
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
...
func TestSelectAccountForCodexModelsSkipsAPIKeyBeforeOAuth(t *testing.T) {
	...
	selection, err := s.selectAccountForCodexModels(context.Background(), []Account{
		newCodexModelsSelectionAPIKeyTestAccount(1),
		oauth,
	})
	if err != nil {
...
func TestSelectAccountForCodexModelsAPIKeyOnlySelectsUpstreamFirst(t *testing.T) {
	s := &OpenAIGatewayService{}

	selection, err := s.selectAccountForCodexModels(context.Background(), []Account{
		newCodexModelsSelectionAPIKeyTestAccount(1),
		newCodexModelsSelectionAPIKeyTestAccount(2),
	})
	if err != nil {
		t.Fatalf("selectAccountForCodexModels returned error: %v", err)
	}
	if selection.APIKeyOnly || selection.Account == nil || selection.Account.ID != 1 {
		t.Fatalf("selection: got %#v, want first API-key upstream", selection)
	}

	selection, err = s.selectAccountForCodexModelsWithExclusions(context.Background(), []Account{
		newCodexModelsSelectionAPIKeyTestAccount(1),
		newCodexModelsSelectionAPIKeyTestAccount(2),
	}, nil, map[int64]struct{}{1: {}, 2: {}})
	if err != nil {
		t.Fatalf("selectAccountForCodexModelsWithExclusions returned error: %v", err)
	}
	if !selection.APIKeyOnly || selection.Account != nil {
		t.Fatalf("selection: got %#v, want built-in catalog fallback", selection)
	}
}
