...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	linkCardRepository := repository.NewLinkCardRepository(client, db)
	linkCardService := service.NewLinkCardService(linkCardRepository, settingRepository, apiKeyService, billingCacheService, concurrencyService, configConfig)
	linkCardHandler := admin.NewLinkCardHandler(linkCardService)
	upstreamBillingProbeService := service.ProvideUpstreamBillingProbeService(accountRepository, accountTestService, settingService, leaderLockCache, db)
	ollamaCloudUsageService := service.ProvideOllamaCloudUsageService(accountRepository, httpUpstream, settingService, secretEncryptor, configConfig, leaderLockCache, db)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, linkCardHandler, upstreamBillingProbeService, ollamaCloudUsageService)
	...
	pointsBridgeRepository := repository.NewPointsBridgeRepository(db)
	pointsBridgeService := service.NewPointsBridgeService(pointsBridgeRepository, billingCacheService, configConfig)
	pointsHandler := handler.NewPointsHandler(pointsBridgeService)
	handlerLinkCardHandler := handler.NewLinkCardHandler(linkCardService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, pointsHandler, handlerLinkCardHandler, idempotencyCoordinator, idempotencyCleanupService)
