...
- The `v0.1.175` compatibility release is `image-update-required`. Back up PostgreSQL and audit `schema_migrations` by full filename/checksum before the maintainer performs the Compose image switch; do not install this migration set through binary hot update.
