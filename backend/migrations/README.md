...
- The `v0.1.176` compatibility release is `image-update-required`. Back up PostgreSQL and audit `schema_migrations` by full filename/checksum, including `221_group_model_pricing.sql`, before the maintainer performs the Compose image switch; do not install this migration set through binary hot update.
