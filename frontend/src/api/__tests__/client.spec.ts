...
describe('API Client', () => {
  ...
  describe('请求拦截器', () => {
    ...
    it('额度卡公共接口不携带 Sub2API 登录令牌', async () => {
      ...
      await apiClient.get('/public/link-cards/me')
  ...
  describe('401 Token 刷新', () => {
    it('额度卡短期会话过期时不跳转登录页或清除 Sub2API 登录态', async () => {
      localStorage.setItem('auth_token', 'dashboard-token')
      localStorage.setItem('refresh_token', 'dashboard-refresh')
      ...
      const adapter = vi.fn().mockRejectedValue({
        response: {
          ...
          data: {
            code: 401,
            reason: 'LINK_CARD_SESSION_INVALID',
            message: 'link card session is invalid or expired',
          },
        },
        config: {
          url: '/public/link-cards/me',
          headers: { 'X-Link-Card-Session': 'expired-session' },
        },
        code: 'ERR_BAD_REQUEST',
      ...
      await expect(apiClient.get('/public/link-cards/me')).rejects.toEqual(
        expect.objectContaining({
          status: 401,
          reason: 'LINK_CARD_SESSION_INVALID',
        })
      ...
      expect(localStorage.getItem('auth_token')).toBe('dashboard-token')
      expect(localStorage.getItem('refresh_token')).toBe('dashboard-refresh')
