...
vi.mock("vue-i18n", async () => {
  ...
  const translations: Record<string, string> = {
    ...
    "admin.settings.openaiFirstResponse.title": "OpenAI 首字优化",
    "admin.settings.openaiFirstResponse.description": "默认关闭。开启后，流式请求在写出客户端前等待首个上游事件，超时会尝试切换同分组其他可用账号。",
    "admin.settings.openaiFirstResponse.timeout": "首事件超时（毫秒）",
