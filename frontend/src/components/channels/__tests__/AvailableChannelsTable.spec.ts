...
describe('AvailableChannelsTable responsive surfaces', () => {
  ...
  it('keeps every model visible for private media channels on both surfaces', () => {
    const wrapper = mountTable({
      rows: [
        {
          name: 'Video Channel',
    ...
    for (const testId of ['desktop-channels', 'mobile-channels']) {
      const surface = wrapper.get(`[data-testid="${testId}"]`)
      expect(surface.text()).toContain('Video Channel')
      expect(surface.text()).toContain('grok-video')
      expect(surface.text()).toContain('seedance-2.0')
    }
  })
