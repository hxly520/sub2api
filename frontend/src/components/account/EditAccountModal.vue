<template>
  ...
  >
    ...
    >
      ...
      <!-- OpenAI API key upstream relay mode -->
      <div
      ...
      >
        <div class="flex items-center justify-between gap-4">
          <div>
            <label class="input-label mb-0">{{ t('admin.accounts.openai.upstreamRelay') }}</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.openai.upstreamRelayDesc') }}
            </p>
          </div>
          <button
            type="button"
            data-testid="openai-upstream-relay-toggle"
            @click="openaiUpstreamRelayEnabled = !openaiUpstreamRelayEnabled"
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
              openaiUpstreamRelayEnabled ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
            ]"
          >
            <span
              :class="[
                'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                openaiUpstreamRelayEnabled ? 'translate-x-5' : 'translate-x-0'
              ]"
            />
          </button>
        </div>
      ...
      <!-- OpenAI OAuth Codex namespace flatten compatibility -->
      <div
...
const openaiUpstreamRelayEnabled = ref(false)
...
const readOpenAIEndpointCapabilities = (credentials?: Record<string, unknown>): OpenAIEndpointCapability[] => {
  ...
  return [...defaultOpenAIEndpointCapabilities]
}
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  openaiUpstreamRelayEnabled.value = false
  ...
  if (newAccount.platform === 'openai' && (newAccount.type === 'oauth' || newAccount.type === 'setup-token' || newAccount.type === 'apikey')) {
    ...
    openaiUpstreamRelayEnabled.value =
      newAccount.type === 'apikey' && extra?.openai_upstream_relay === true
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.account.platform === 'openai' && (props.account.type === 'oauth' || props.account.type === 'setup-token' || props.account.type === 'apikey')) {
      ...
      if (props.account.type === 'apikey' && openaiUpstreamRelayEnabled.value) {
        newExtra.openai_upstream_relay = true
      } else {
        delete newExtra.openai_upstream_relay
      }
