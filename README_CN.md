...
本仓库的当前代码基线为 Sub2API `v0.1.168`，官方兼容合并提交为 `d30c42da`；生产环境仍运行 `v0.1.164` / `08fbef836`，候选镜像的构建、上传或服务器缓存均不代表生产容器已经切换。私有分支在官方基线上保留协议缓存兼容、账号调度与首响应、图片/视频任务、媒体下载代理、媒体余额冻结与后台核销、独立积分与签到系统、未登录公开首页、可用渠道展示、KeyingPay V2、CC Switch 导入和 Cloudflare/Nginx 源站加固等扩展。二开遵循“官方实现为主、私有能力兼容补充”的原则。
