...
- 截至 `2026-07-31 23:27 CST`，生产 Sub2API 仍为 `0.1.169-04a19ca082ee` / revision `04a19ca082ee43853573795d1385727bd38f20e9`，容器 `c37a7a014997...`；积分服务已独立更新为 `0.1.169-f79803bb73d6` / revision `f79803bb73d659e36627d6f716aab065ff4d56a6`，容器 `05f43434fc20...`。两者均 healthy、restart count `0`；Sub2API 最终候选只加载到服务器镜像缓存，仍由维护者手工切换。生产版本必须以运行容器 OCI revision 为准，不能只看仓库 `main` 或服务器镜像缓存；详见 [`PRODUCTION_DEPLOYMENT_20260731_CN.md`](PRODUCTION_DEPLOYMENT_20260731_CN.md)。
...
| `04a19ca08` | v0.1.169 最终发布源码 | Gemini 非流式 SSE 聚合收到正式 `finishReason` 后立即完成；当前生产 Sub2API 与积分镜像 revision |
...
- 普通用户采用双服务预览白名单分阶段开放。Sub2API `points_system.enabled=false` 且 `preview_user_ids: [1]`，积分服务 `POINTS_USER_ACCESS_MODE=preview` 且 `POINTS_USER_PREVIEW_IDS=1` 时，仅用户 ID 1 显示积分菜单、通过 `/points` 路由、获得 user ticket 并继续使用积分 user session；其余用户即使依赖陈旧公共设置、手工构造路由/launch 请求或持有收窄前旧 cookie，也必须被拒绝并清除积分 cookie。两份完整白名单不得下发浏览器，认证接口只返回当前用户专属的 `points_system_access` 布尔值。预览授权不绕过桥接配置、生效 policy、角色、一次性票据、session 或积分服务四层关闭态校验；调试验收完成后必须在同一维护变更中把 Sub2API `enabled=true`、积分服务 mode=`all` 并清空积分预览名单，才开放全体用户。
...
- 普通用户开放必须同时满足 Sub2API 的逐用户授权与积分服务当前生效策略完整启用。逐用户授权为“全局 `enabled=true`”或“全局关闭且当前用户在 `preview_user_ids`”二选一；积分服务继续在 user ticket、页面、静态资源和 API 四层执行关闭态校验，旧会话与手工 URL 不能绕过。截至 `2026-07-31`，policy v3 已启用、历史回算已成功且签到关闭；本轮预览配置固定为 `enabled=false`、`preview_user_ids: [1]`，全站开放必须是后续显式变更。
...
- 当前生产 Sub2API 为 `0.1.169-04a19ca082ee`；积分服务为 `0.1.169-f79803bb73d6`。Sub2API 当前版本包含容量精确重试、跨协议会话中断修复、历史激活门禁、新未登录首页、同源上传 Logo、嵌入式双工作区、用户名显示和软删除会话失效；最终候选新增管理员配置新标签页、用户 1 预览授权、用户 iframe 实时主题同步及前端逐用户拒绝优先级，仍只能由维护者手工切换。积分服务已上线票据交换与既有 session 的双重预览门禁。policy v3 仍为比例 `10.00:1`、刷新 `00:05`、签到关闭；历史作业仍为原成功记录。当前事实见 [`PRODUCTION_DEPLOYMENT_20260731_CN.md`](PRODUCTION_DEPLOYMENT_20260731_CN.md)，`2026-07-30` 文档只作为历史发布记录。
- 最终 Sub2API 候选 GHCR digest 为 `sha256:efeac9309f33a9ccd204932ed1144955641a78260982c3041842fb89c71caa49`；当前生产积分 GHCR digest 为 `sha256:d5325808dc2950632f4d4f98ff87a167265d0dbf2a45e9f0b8e446bd51c96876`。服务器缓存中的 image ID、传输 archive SHA256 和运行容器 revision 必须继续分别核对，不能把任一值当作另一值。
