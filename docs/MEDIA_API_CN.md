...
- 余额计费会在创建前按当前渠道定价预留预计费用。可用余额不足时返回 `402`，请求不会提交到生成服务；成功后按实际费用结算并退还差额，失败不扣费。
...
- 平台在图片/视频创建前校验并预留预计费用，多个并发请求不能共同透支同一余额。
- `402` / `MEDIA_INSUFFICIENT_BALANCE` 表示可用余额不足，客户端充值后应创建新请求；该错误不会产生生成任务。
- 图片/视频创建失败、取消或过期不记录成功 usage。
- 异步任务在终态成功时按创建时保存的价格快照结算，并以任务 ID 幂等，只记录一次。
- 实际费用低于预留费用时自动退回差额；查询、下载和重复轮询不会再次冻结或扣费。
...
8. 余额不足在调用上游前拒绝；并发创建不能透支；成功结算后冻结余额归零或只保留其他在途任务金额。
