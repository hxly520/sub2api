...
| --- | --- | --- | --- |
| Sub2API | `0.1.168-339422728b2c`，revision `339422728b2ceb87b4a81bb08229d370c4ca589d`，容器 `bfd162bb...`，healthy | 官方 `v0.1.168` 加私有兼容提交；最终候选 commit 以本轮推送后的 `main` 为准 | 只构建、推送并上传/缓存候选；维护者手工切换，自动化不得替换或重启 |
| 积分服务 | `0.1.168-28e760bc8c6d`，revision `28e760bc8c6d66414595ef2af213d301a423acf2`，容器 `7b745c53...`，healthy | `874255bcd` 之后的嵌入式大屏与中文状态优化；最终 commit 与 Sub2API 候选相同 | 先备份 `points` schema，可独立更新并验证；不得重跑历史基线，且不得改动 Sub2API 容器 |
| exact-root 首页 | Nginx 从宿主 `/home/api/sub2api-deploy/public/index.html` 提供 | 仓库 `deploy/public-landing/index.html` 冷灰/电蓝新版 | 备份后原子替换并核对 SHA256；不 reload Nginx，不重启 Sub2API |
...
- 本轮未登录首页、同源上传 Logo、同步生图冻结时限、连续自然日积分曲线、全量签到发放汇总和中文状态补充在 `874255bcd` 后的工作树中；镜像 tag、OCI revision 和 digest 必须使用最终推送 commit，禁止用 `874255bcd` 猜测最终制品。
...
- 用户工作区为简洁中文大屏，只显示总积分、昨日积分、今日/累计签到赠送、7/30/90 日积分趋势、个人积分记录和签到奖励记录。签到关闭时按钮必须清晰锁定，页面不得出现策略、全站用户列表、发放重试、冲正或其他管理员控件。
- 管理员工作区提供运行概览、全站用户积分明细、策略管理和签到发放任务。用户明细以未删除 Sub2API 用户为基准，显示总/昨日积分、累计/昨日成功消费和昨日结算状态，零消费用户补零显示。
...
- 本轮只修改未登录 Vue `/home` 和 exact-root 静态首页，不改登录后的 Dashboard、用户导航或管理员业务页面。文案只描述功能、稳定性和管理能力，不出现具体国外模型或商业中转宣传名称。
...
- 后续候选要求 Responses 只接受 `response.completed/done` 为成功终态。仅 `[DONE]` 或 EOF 不再返回零 usage 的静默成功：未输出语义正文时进入安全换号，已输出部分正文且客户端仍连接时补 `response.failed`；客户端已断开时继续读取上游完成 usage/计费收口并写 `openai.responses_client_disconnected` 结构化日志。已取消请求中的 `response_id` 账号绑定改用不继承取消信号的上下文完成。
- 本节全部为只读排查和候选代码结论。运行中的 Sub2API 仍是 `0.1.168-339422728b2c`，没有替换或重启；修复要等维护者手工切换最终候选镜像后才生效。

## 9. 发布与验证

本轮源代码验证已完成：后端 `go test -tags=unit ./... -count=1`；前端 ESLint、typecheck、全量 Vitest 和生产 build；积分 `go test ./...`、`go vet ./...` 和 build；桌面/移动 Playwright 无横向溢出且趋势图非空。镜像工作流仍须在最终 `main` commit 上重新执行自己的测试和冒烟，不能用本地通过替代制品证据。
...
5. 更新积分前新建并校验 `points` schema 备份，记录旧容器、镜像和账户/快照/账本计数；仅对积分服务执行 `docker compose up -d --no-deps points-system`。更新后核对 21 表/3 迁移、policy v3 和历史 job ID 不变，对比更新前计数与期间正常调度增量，确认无重复历史账本、无异常减少、`needs_review=0`、签到/发放仍为 0，并断言 Sub2API 容器 ID、启动时间和镜像引用完全不变。
6. exact-root 首页按第 5 节独立原子发布。它不依赖镜像切换，也不需要 Nginx reload，但线上 SHA256 必须与仓库候选一致。

截至本文首次写入，下一候选的最终 commit、镜像 tag 和 digest 尚未生成。构建完成后必须把这些值追加到本节或同日续录，禁止把源码 commit、旧 registry digest、image ID 或 archive SHA256 互相冒充。
...
- 后续合并官方版本时，以官方结构与通用行为为主，同时逐项回归媒体冻结核销、积分同库、历史积分、双工作区、管理员用户明细、签到发放安全、容量精确重试、未登录首页和同源上传 Logo。
