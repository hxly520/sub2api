...
> 状态说明：截至 `2026-08-07`，功能代码已进入私有 `main` 提交 `71dff449192f982008bc333940636827a9f88786`，默认关闭的 Linux/amd64 开发候选镜像已经构建并发布；候选尚未上传服务器、切换容器、执行迁移或接入真实资金验收。生产运行态仍以 [`PRODUCTION_OPERATIONS_CN.md`](PRODUCTION_OPERATIONS_CN.md) 第 0 节为准；以下内容是必须继续验证的候选契约，不代表迁移 `194_link_cards.sql` 已进入生产。
