...
func TestPostgresAdminUserPointsArePagedAndJoinedToYesterday(t *testing.T) {
	...
	if _, err := fixture.db.Exec(ctx, `CREATE TABLE users(
		id BIGINT PRIMARY KEY,email TEXT NOT NULL DEFAULT '',deleted_at TIMESTAMPTZ
	)`); err != nil {
	...
	for _, row := range []struct {
		...
		{userID: 101, loginEmail: "alice@example.com", points: 100, spend: 1_000_000},
		{userID: 202, loginEmail: "bob@example.com", points: 900, spend: 9_000_000},
		{userID: 303, loginEmail: "carol@example.com", points: 500, spend: 5_000_000},
	} {
		if _, err := fixture.db.Exec(ctx, `INSERT INTO users(id,email) VALUES($1,$2)`, row.userID, row.loginEmail); err != nil {
	...
	if _, err := fixture.db.Exec(ctx, `INSERT INTO users(id,email) VALUES(404,'zero@example.com')`); err != nil {
	...
	if page.Items[0].UserID != 202 || page.Items[0].LoginEmail != "bob@example.com" || page.Items[0].YesterdayPointsHundredths != 200 ||
	...
	if page.Items[1].UserID != 303 || page.Items[1].LoginEmail != "carol@example.com" || page.Items[1].SnapshotStatus != "missing" ||
	...
	if last.Total != 4 || len(last.Items) != 2 || last.Items[0].UserID != 101 || last.Items[0].LoginEmail != "alice@example.com" ||
		last.Items[1].UserID != 404 || last.Items[1].LoginEmail != "zero@example.com" || last.Items[1].TotalPointsHundredths != 0 ||
		last.Items[1].TotalSpendMicroUSD != 0 {
		t.Fatalf("unexpected final page: %+v", last)
	}

	loginEmail, err := fixture.store.LoginEmail(ctx, 202)
	if err != nil || loginEmail != "bob@example.com" {
		t.Fatalf("lookup login email = %q, err=%v", loginEmail, err)
	}
	...
	if len(grants) != 1 || grants[0].LoginEmail != "bob@example.com" || grants[0].Grant.UserID != 202 {
	...
	if _, err := fixture.store.LoginEmail(ctx, 202); err == nil {
		t.Fatal("soft-deleted user still resolved to a login email")
	}
