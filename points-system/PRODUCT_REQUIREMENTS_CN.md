...
- 截至 `2026-08-01 22:34 CST`，Sub2API 仍为维护者手工切换的 `0.1.169-f79803bb73d6` / revision `f79803bb73d659e36627d6f716aab065ff4d56a6`，容器 `63d320fbf6ca...`；积分服务已独立切换为 `0.1.169-bee059a1cec5` / revision `bee059a1cec5d0eb1a6d022d766670489dcf484d`，容器 `43bde62f3fa2...`。两者均 healthy、restart count `0`，积分切换前后 Sub2API 的容器 ID、启动时间和镜像引用未变化。后续积分服务可按本文边界独立更新，但自动化不得替换或重启 Sub2API；镜像 digest、image ID 与传输归档 SHA256 必须在每次发布记录中分别记载，不能互相替代。
...
- 历史分阶段调试曾保持 Sub2API `POINTS_SYSTEM_ENABLED=false`、`POINTS_SYSTEM_PREVIEW_USER_IDS=1`，同时把积分服务设置为 `POINTS_USER_ACCESS_MODE=preview`、`POINTS_USER_PREVIEW_IDS=1`；该阶段已于 `2026-08-01 22:34 CST` 结束。当前两端部署门禁均为全体模式且预览名单为空，普通用户是否显示积分中心只由当前生效 policy 的 `enabled` 控制。无论全体还是预览模式，积分服务仍需在票据交换和每一次既有 user session 请求复核门禁，防止陈旧 cookie 绕过；两份完整名单均不得返回浏览器。签到保持关闭。
