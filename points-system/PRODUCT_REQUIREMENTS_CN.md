...
- 用户工作区 `/app/` 与管理员工作区 `/admin/` 必须使用独立中文页面和独立脚本。普通用户不得下载管理员脚本、打开管理员页面或调用 `/api/v1/admin/*`，管理员也不得调用普通用户账户、积分、签到或赠送 API；角色不匹配一律返回 `403`，不能在同一页面用前端显隐混合两类能力。只有用户工作区通过 Sub2API `/points` 右侧内容区 iframe 嵌入，左侧继续使用 Sub2API 导航及其上传 Logo；积分主题类只能附加在右侧内容区，不得改变官方侧栏。`ui_mode=embedded` 只控制展示，不得参与角色或授权判定。父页通过 `sub2api:points-theme` 实时同步明暗主题，子页只在消息 source 为 parent、Origin 精确命中 `POINTS_EMBED_PARENT_ORIGIN` 与 `POINTS_EMBED_PARENT_ORIGINS` 合并后的允许列表且值为 `light|dark` 时应用。积分服务的 CSP `frame-ancestors` 只能包含同一组精确父 Origin，禁止通配符及冲突的 `X-Frame-Options`；ready 消息只能逐一发送到这些精确 Origin，禁止使用 `*`。
- 当前 policy v7 于 `2026-08-02` 生效，`enabled=true`、积分比例 `1 U = 10.00`、刷新 `00:05`、`consumer_only/yesterday`、最低昨日消费 `1 U`、每天最多一次，按昨日原始成功余额消费使用 `[1,10) U -> 1%-5%`、`[10,50) U -> 2%-5%`、`[50,100) U -> 3%-5%`、`[100,+∞) U -> 4%-5%` 四档，三个金额上限为 `NULL`。它是经明确授权从 v5 原子复制的当日启用版本；原 v5 不可变并于 `2026-08-03` 接管，后续策略修改仍必须次日生效。首次全历史基线作业仍是唯一成功基线，禁止再次 plan/apply。
- 积分中心和签到部署门禁均为 `all`，两份 preview ID 为空。所有有效用户可进入积分中心并看到签到能力，但 `checkin_available` 仍必须由昨日消费最低值、快照、阶梯和每日次数计算；低于 `1 U` 的用户不得签到，也不得创建余额 grant。服务仍须在票据交换和每次既有 session 请求复核门禁，完整配置不得返回浏览器。
- 用户 1 原 `3.08 U` grant 已真实冲正，关联 reversal 实际扣回 `-3.08 U`，原记录保留为 `reversed`。v5 真实重测按昨日消费 `86.890694 U` 命中 `[50,100) U` 档，安全随机值 `35,820 PPM`，理论区间 `2.60-4.34 U`，新 grant 实发 `3.11 U` 且为 `settled`；同幂等键重放不重复入账，新键再次签到返回 `409`。
- 用户界面的今日赠送和累计赠送必须只统计已到账且未冲正 grant；原签到金额和冲正状态继续保留在不可变明细中。限次、单用户 cap 和平台 cap 的安全计算仍按已接受签到预留金额，不得因为 pending 或 reversed 不显示在净额中而释放当日次数或形成并发超发窗口。当前用户 1 的真实接口净额为 `3.11 U`、签到计数为 2、不可再次签到。
- Sub2API `1a4a690dd999` 已完成 credit/reversal 非空审计验收，临时兼容触发器与函数已删除，`audit_logs.request_body` 继续保持 `NOT NULL`；新 credit/reversal 审计正文均非空。
